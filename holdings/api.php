...
$format = test_input($_GET["format"], "format") ; // inst_id or null
...
if ($count_rowCount == 0 ) { //no search results in bib_info
    ...
    if ( count($alt_lib) > 0) { // if there was an alt oclc number search
        ...
        if ($mapped_rowCount > 0) {
            ...
            showResults($mapped_Results, $newsearch, $pagination, $end, $format, $db);
            echo $message_text;
...
<?php
function showResults ($entries, $newsearch, $pagination, $end, $format, $db) {
